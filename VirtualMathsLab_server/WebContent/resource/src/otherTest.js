...
//					getMediaType(allData);
	...
	OT.createPracticeTest = function(topicId, topicName, testTypeId, data ) {
...
//					$("#main-div").html("<div class='col-xl-12 col-md-12 col-sm-12 alert alert-danger' style='text-align:center;'>No questions in the database to display....</div>");
	...
	OT.otherTestHeading = function(firstName, topicName, topicId, testTime) {
...
//		 var topicNm = topicName.split("-");
		...
		$("#other-test-heading").html(heading);
//		alert(parseInt(testTime));
		OTM.otherTestTimer(parseInt(testTime), topicId);
	}
	
	OT.otherTestMessage = function(msg) {
		$("#other-test-main-div").html("<div class='col-xl-12 col-md-12 col-sm-12 alert alert-danger' style='text-align:center;'>"+msg+"</div>");
	}
	
	
	OT.startOtherTest = function(testData, firstName) {
		
		OT.TESTDATA = testData;
		OT.TISID = testData.TISID;
		console.log(testData);
		
		var testTime = testData.data["time" + testData.TOPICS[0]];
		
//		OT.otherTestHeading(firstName, testData.topics, testData.TOPICS[0], testTime);
		OT.otherTestHeading(firstName, testData["topics"+ testData.TOPICS[0]], testData.TOPICS[0], testTime);
		
		OT.otherTestMainDiv(testData);
	}
	
	
	OT.otherTestMainDiv = function(testData) {
		console.log(testData);
		var topicId = testData.TOPICS[0]; // This is because in practice test has only one topic at at time
		
		OT.renderActualQuestionDiv(testData, topicId);			
//		OT.switchtoThisQues(testData, topicId);
	}
	
	
	OT.renderActualQuestionDiv = function(testData, topicId) {
//		$(".modal-backdrop show").css("display", "none");
//		$("#Loading").css("display","none");
		 topicObj = testData.topicObj;
		 topicObjLen = Object.keys(topicObj).length;
//		 alert(topicObjLen);
		$("#content").css({"margin-left":"0%"});
		$("#sidebar .custom-menu .btn").addClass({"margin-left":"-8px"});
		$("#sidebar").addClass("active");
//		var topicNm = testData.topics.split(">>"); 
		var topicNm = testData["topics"+ topicId].split(">>"); 
		
		OT.topicName = topicNm[0].split("(");
		var quesGroupHtml = '';
		...
		for (var i = 0; i < testData.data["TOPIC"+topicId].length; i++) {
			...
			if(testData.data["TOPIC"+topicId][i].QUES.length > 1){
				...
				for (var j = 0; j < testData.data["TOPIC" + topicId][i].QUES.length; j++) {
					...
					quesGroupHtml  +='<span id="quesNo_'+(i+1)+'_'+(j+1)+'" onclick="com.coep.test.otherTest.switchtoThisQues('+i+ ',' +j+ ',\'qGrp'	+ (i + 1)+ '_'+ (j + 1)	+ '\',\'quesNo_'+ (i + 1)+ '_'+ (j + 1)	+ '\','+topicId+')" tabindex="qGrp'+(i+1)+'_'+(j+1)+'"' 
						...
						if(testData.data["TOPIC" + topicId][i].QUES[j].ANSTYP == 5 ){
							...
							for (var r = 0; r < testData.data["TOPIC" + topicId][i].QUES[j].ANS.length; r++) {
								if(index == testData.data["TOPIC" + topicId][i].QUES[j].GANSID[0]["GANSID"+(r+1)]){
									OT.temp11 = '';
									OT.temp11 = ' class="quesPanel quesPanelchecked">'
									
										break;
								}else{
									OT.temp21 = '';
									OT.temp21 =  ' class="quesPanel">'
								}
							}
...
//							+ 'Answered'
		...
		quesGroupHtml += 'Q'+(i+1)+'.'+(j+1)
					...
					quesGroupHtml  +='<span id="quesNo_'+(i+1)+'" onclick="com.coep.test.otherTest.switchtoThisQues('+i+ ',0,\'qGrp'+ (i + 1)+ '\',\'quesNo_'+ (i + 1)+ '\','+topicId+')"  tabindex="qGrp'+i+'"'
						...
						for (var p = 0; p < testData.data["TOPIC" + topicId][i].QUES.length; p++) {
							...
							for (var r = 0; r < testData.data["TOPIC" + topicId][i].QUES[p].ANS.length; r++) {
								if(index == testData.data["TOPIC" + topicId][i].QUES[p].GANSID[0]["GANSID"+(r+1)]){
									OT.temp1 = '';
									OT.temp1 = ' class="quesPanel quesPanelchecked">'
										break;
								}else{
									OT.temp2 = '';
									OT.temp2 =  ' class="quesPanel">'
								}
							}
...
//					+'class="quesPanel">'
		...
		+'</div>'// quesGroupMedia end
			...
			+'<div id="other-test-footer" class="col-sm-12 col-md-12 col-lg-12  col-xl-12"></div>'
			+'</section>'
...
//		+'<span class="quesPanel">1</span> Question Paper'
		+ '<div  onClick="javascript:com.coep.test.AlertMessage.showQuestionPaperClick('+topicId+')" data-toggle="modal" data-target="#AlertMesConfirm3" title="Show Question Paper" id="QuePaperHeader"><i class="fa fa-question-circle" aria-hidden="true"></i> Question Paper <span class="marathi-text" >(प्रश्नपत्रिका)</span></div>' 
		+'<br/>'
		...
		if(Object.keys(topicObj)[topicObjLen - 1] == topicId){
			...
			quesGroupHtml +=  '<span onClick="javascript:com.coep.test.AlertMessage.confirmationToGoNextTopic('
				+ OT.subTestFlag
				+ ','
				+ topicId
				+ ', \'nxt\')" title="NEXT TOPIC" id="submitBtn" class="btn btn-success"  data-toggle="modal" data-target="#AlertMesConfirm">NEXT TOPIC <span class="marathi-text hideFormobile" >( परीक्षा समाप्त करा)</span></span>'
				
				+'</div>'
		}
		
		
		quesGroupHtml += '</div>' // left question section div end
		
			
		$("#other-test-main-div").html(quesGroupHtml);	
		
		$("#alert").hide();
		
		var footer = '<div id="" class="">' // nextPrevButtonDiv  div start
			+'<span class="previous" id="switchtoPrevQuesSpan" ><a class="btn btn-info" href="javascript:com.coep.test.otherTest.switchtoPrevQues()" style=""> <i class="fa fa-arrow-circle-left" aria-hidden="true"></i> <b class="hideFormobile">Previous <span class="marathi-text" >( मागचा प्रश्न )</span></b></a></span>'
...
//					+ ', \'nxt\')" title="FINISH TEST" id="submitBtn" class="btn btn-success"  data-toggle="modal" data-target="#AlertMesConfirm">FINISH TEST <span class="marathi-text hideFormobile" >( परीक्षा समाप्त करा)</span></span>'
					...
					+ '<span class="next" id="switchtoNextQuesSpan"><a class="btn btn-info" href="javascript:com.coep.test.otherTest.switchtoNextQues()"><b class="hideFormobile">Next <span class="marathi-text" >( पुढील प्रश्न )</span></b> <i class="fa fa-arrow-circle-right" aria-hidden="true"></i></a></span>'
		+'</div>' // nextPrevButtonDiv  div end
		
		$("#other-test-footer").html(footer);	
		
		if (testData.data["TOPIC" + topicId][0].QUES.length > 1) {
			
//			alert(testData.data["TOPIC" + topicId][0].QUES.length);
			OT.switchtoThisQues(0, 0, 'qGrp1_1', 'quesNo_1_1',topicId);
			MJ.renderMathJax();
			fristQues = "quesNo_1_1";
			setTimeout(function(){ OT.callFirstQues(); }, 1000); 
		} else {
//			alert("In");
			OT.switchtoThisQues(0, 0, 'qGrp1','quesNo_1', topicId);
			MJ.renderMathJax();
			fristQues = "quesNo_1";
			setTimeout(function(){ OT.callFirstQues(); }, 1000); 
		}
	...
	OT.switchtoThisQues = function(j, k, qGrp, qTxtTemp, topicId) {
...
//			OT.quesPaper += qtxtTemp +'<br/>' ;
		...
		OT.renderOptions(j, k, topicId, ansType, qTxtTemp, testData);
		com.coep.test.mathJax.renderMathJax();
		
		/// previous question button
		OT.switchtoPrevQues = function() {
			...
			OT.switchtoThisQues(preNextIndex1[temp], preNextIndex2[temp],
...
//													"tagged" :OT.tagged,
	...
	OT.mediaTypeQuestion = function(j, k, testData, topicId) {
		...
		if (testData.data["TOPIC" + topicId][j].MEDTYP == 1) {
			...
			+'<source id="audio" src="'+OT.baseURL +'media/getImage?mediaID='+testData.data["TOPIC" + topicId][j].MED+'&questionGroupId='+testData.data["TOPIC" + topicId][j].QGID+'" type="audio/mpeg">'
...
//			$("#audio").attr("src",  );
			...
			+'<source src="'+OT.baseURL +'media/getImage?mediaID='+testData.data["TOPIC" + topicId][j].MED+'&questionGroupId='+testData.data["TOPIC" + topicId][j].QGID+'" type="video/mp4">'
...
//			$("#videoQuestion").attr("src", );
			...
			+'<img src="'+OT.baseURL +'media/getImage?mediaID='+testData.data["TOPIC" + topicId][j].MED+'&questionGroupId='+testData.data["TOPIC" + topicId][j].QGID+'" id="imgQues" max-width="75%">' // class="ApproveQueImg" question images
			+'</div>'
...
//			$("#imgQues").attr("src", );
	...
	AM.showQuestionPaperClick  = function(topicId) {
		
		var testData = 	OT.TESTDATA;
...
//						+ '<button type="button" class="btn btn-danger" data-dismiss="modal">No</button>'
			...
			$("#other-test-main-div").append(AlertMesConfirm);
		    com.coep.test.mathJax.renderMathJax();
//			AlertComfirmYes = function()
//			{
//				OTM.onFinishTest();
//				AlertComfirmFlag = true;
//				OT.submitTest(subTestFlag, key, status);
...
//			}
	...
	OT.submitTest = function(subTestFlag, key, status) {
...
//			$(this).prop('disabled', true);
		...
		OT.finishTest(testData.TISID, completeTestStatus);
	}
	
	
	// confirmation to go to next topic codde start
	AM.confirmationToGoNextTopic = function(subTestFlag, key, status) {
		var AlertComfirmFlag = false;
			AlertMsg = 	 'Do you want go to the next topic.?<br><span style = "color:red; font-size:16px;" >Once you contiune you can not go back to the previous topics.</span> <span class="marathi-text" >( तुम्हाला चाचणी परीक्षा समाप्त करायची आहे का? )</span> ';
			var AlertMesConfirm = ''
				AlertMesConfirm +=  '<div class="container-fluid">'
				+ '<div class="row">'
				+ '<div  class="col-sm-12 col-md-12 col-lg-12 col-xl-12">'
...
//					OTM.onFinishTest();
					...
					OT.saveCurrentTime();
					OT.selNextTopic(subTestFlag, key, status);
					return AlertComfirmFlag;
				}
	...
	OT.selNextTopic = function(subTestFlag, key, status) {
		
//		var loader = '<div class="overlay" id="Loading">'
//			 + '<div class="overlay__inner">'
//			 + '<div class="overlay__content"><span class="spinner"></span><br/><span class="loading">Next topic....</span></div>'
//			 + '</div>'
//			 + '</div>'
//			 
//			 $("#otherTestDiv").append(loader);
		...
		var topickeys = [];
		topickeys = Object.keys(topicObj);
		var tId = key+"";
		
		var nxtTopicId = 0;
//		topicObjLen
		if (topickeys[topicObjLen - 1] != tId) {
//			clearInterval(ST.interval);
//			clearInterval(timerInterval);
			OTM.onFinishTest();
			nxtTopicId = topickeys.indexOf(tId) + 1;
			nxtTopicId = topickeys[nxtTopicId];
			OT.renderActualQuestionDiv(testData, nxtTopicId);
			var timeDuration = testData.data["time" + nxtTopicId];
			OTM.otherTestTimer(parseInt(timeDuration), nxtTopicId);
...
//		$(".modal-backdrop show").css("display", "none");
	...
	OT.testInstanceVOCall = function(testInstanceVOArr) {
		
		if (testInstanceVOArr.length != 0) {
			$.ajax({
				type : "PUT",
				url : OT.baseURL + "testInstance",
	...
	OT.finishTest = function(TISID,status) {
		...
		setTimeout(function() {
			$.ajax({
				...
				success : function(data) {
					...
					OT.completeTest(testJson, data, status, TISID);
...
//					window.location.reload(true);
	...
	OT.completeTest = function(testJson, summaryData, status, TISID) {
		clearInterval(OT.saveCurrentTime());
		...
		$.ajax({
			...
			success : function(data) {
				...
				if (data.done == true) {
					OT.summaryReport(summaryData, TISID);
				}
...
//		setTimeout(AP.renderAttemped(data,status), 1000);
	...
	OT.saveCurrentTime = function() {

		var CTJson = {
			"testInstanceStateId" : OT.TISID,
			"currentTestTime" : OT.CT
		};

		$.ajax({
			type : "POST",
			url : OT.baseURL + "testInstanceState/update/currentTime",
	...
	OT.summaryReport = function(data, tISID) {
//		console.log(data);

		var topicNm = data.TOPICNM.topicNm.split(">>");
		
		$("#other-test-heading").hide();
		$("#other-test-footer").hide();
		$("#other-test-main-div").html('');
...
//				+ ' <div id="timeOut" class="alert alert-warning cust-margin" role="alert" hidden><span class="glyphicon glyphicon-remove-circle  alert-warning "></span><B>&nbsp;&nbsp;Your session has timed out and your answers has been saved successfully..</B></div>'
			...
			candidateSummaryHtm +=  ' <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12 " >'
				+'<div class="row">'
//				+'<div class="col-sm-12 col-md-12 col-lg-12 col-xl-12 hideFormobile" id="topicDetails" style="" ><span >TOPIC</span><b id="talkbubble"></b><span >'+tname[0]+'</span><b id="talkbubble"></b><span > '+tname[1]+'</span></div>'
//				+'<div class="col-sm-12 col-md-12 col-lg-12 col-xl-12 hideFormBigscreen" id="topicDetails" style="text-align: left;" ><span >TOPIC <i class="fa fa-angle-double-right TopicArrow" aria-hidden="true"></i>'+tname[0]+' <i class="fa fa-angle-double-right TopicArrow" aria-hidden="true"></i> '+tname[1]+'</span></div>'
			...
			+ '</div>'
			$("#other-test-main-div").html(candidateSummaryHtm);
			
			var userId = data.userId;
			
			OT.MoveDatafromInstaceToCompletion(OT.baseURL+"testInstanceToCompletionForUser?userId="+userId+"&tISID="+tISID, "MoveDataToCompletion", userId, tISID);
	}
	
	
	  OT.MoveDatafromInstaceToCompletion = function(url, userId, tISID){
				...
				$.ajax({
						...
						success : function(data) {                 
							if (data.done != false) {
		//						console.log(data);
								OT.showTestReport(OT.TESTDATA, data.USERANSDATA, data.ANSDATA, data.GivenTIME,  data.TakenTIME, data);
	...
	OT.showTestReport = function(data, userAnsData, ansData, GivenTIME, TakenTIME, PERDATA) {
//		console.log(PERDATA);
		console.log(data);
...
//			 	+ TakenTIME
//			 	+ '</span>' // needs to be corrected on java side for this particularly
			...
			var topicID = data.TOPICS[k];
			quesHtm += '<hr> Topic-'+(k+1)+') '+testData.topicObj[topicID]+'</hr>';
			...
			for (var j = 0; j < testData.data["TOPIC" + topicID][i].QUES.length; j++) {
				...
				if (userAnsData[cnt].USERANS != "Correct") {
					quesHtm += '<tr  id="'+i+'-'+j+'-'+k+'" >' 
						...
						if(testData.data["TOPIC"+topicID][i].QUES.length > 1){
							quesHtm += '<td width="5%" style="text-align: center;">Q' + (i + 1)+'.'+(j+1) +'<br>Topic-'+(k+1)+''
						}else{
							quesHtm += '<td width="5%" style="text-align: center;">Q' + (i + 1) +'<br>Topic-'+(k+1)+''
						}
					...
					+ '<span class="resultBtn btn btn-success" onClick="javascript:com.coep.test.AlertMessage.showSolutionOnClick('+i+','+j+','+k+',\'active\')" data-toggle="modal" data-target="#AlertMesConfirm2-'+i+"-"+j+"-"+k+'" title="Click here for Solution (उत्तरासाठी येथे क्लिक करा)" class="btn btn-success AcT_ArchIcon" style="">'+ userAnsData[cnt].USERANS + '</span>'
					+ '</td>' + '</tr>'
...
//		+ '<div ></div>'
		...
		// restart the test 
//		testData["topics"+ testData.TOPICS[0]]
//		TOPICS[0] topics845 
//		console.log(testData["topics"+ testData.TOPICS[0]]);
//		var topicNmForCurrTest = testData["topics"+ testData.TOPICS[0]].split(">>"); 
		 ...
		 var w = $(window).width();
		    if(w < 768){ 
		     ...
		     $(window).resize(function() {
			  ...
			  AM.showSolutionOnClick = function(i,j,k, temp) {
			    ...
			    var topicID = data.TOPICS[k];
					...
					var AlertMesConfirm = ''
						...
						+ '<div  class="col-sm-12 col-md-12 col-lg-12 col-xl-12">'
						//Alert modal start
								+ '<div class="modal SolutionModel" id="AlertMesConfirm2-'+i+"-"+j+"-"+k+'">'
//													id="AlertMesConfirm
								+ '<div class="modal-dialog ">'
								+ '<div class="modal-content">'
...
//													 +' </p>'
										...
										+ OT.ansData[((testData.data["TOPIC" + topicID][l].QUES[m].ANSTYP) - 1)].ATN 
										+ '</i></b></div><hr>'
										+ '<div class="table-responsive">'
...
//					                + '<button type="button" class="close" data-dismiss="modal">OKAY</button>'
						...
						$("#otherTestDiv").append(AlertMesConfirm);
				     	com.coep.test.mathJax.renderMathJax();
				} 
...
})(com.coep.test.otherTest, com.coep.test.otherTestTimer, com.coep.test.AlertMessage, com.coep.test.mathJax, com.coep.test.addProblem, com.coep.test.home);